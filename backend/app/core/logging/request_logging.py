...
class RequestLoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Any) -> Any:
        ...
        logger.info("HTTP Request", **request_info, trace_id=request_id)
        ...
        try:
            ...
            if response.status_code >= 400:
                logger.warning(
                    "HTTP Response (Error)", **response_info, trace_id=request_id
                )
            else:
                logger.info("HTTP Response", **response_info, trace_id=request_id)
