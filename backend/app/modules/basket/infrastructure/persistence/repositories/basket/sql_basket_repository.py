...
async def _extract_value(value, depth=0, max_depth=3):
    ...
    if depth >= max_depth:
        logger.error(f"Maximum recursion depth ({max_depth}) reached - this should not happen in production!")
        return value
    ...
    if inspect.iscoroutine(value):
        try:
            ...
            if depth < max_depth - 1 and (inspect.iscoroutine(value) or hasattr(value, '__await__')):
    ...
    is_mock = (
        hasattr(value, '_mock_name') or 
        hasattr(value, '_spec_class') or
        (hasattr(value, '__class__') and 'Mock' in str(value.__class__.__name__))
    )
...
class SqlBasketRepository(IBasketRepository):
    ...
    async def get_basket(
        ...
        session_is_mock = (
            hasattr(self.session, '_mock_name') or 
            hasattr(self.session, '_spec_class') or
            (hasattr(self.session, '__class__') and 'Mock' in str(self.session.__class__.__name__))
        )
        if session_is_mock:
            error_msg = (
        ...
        try:
            ...
            is_mock = (
                hasattr(basket_orm, '_mock_name') or 
                hasattr(basket_orm, '_spec_class') or
                (hasattr(basket_orm, '__class__') and 'Mock' in str(basket_orm.__class__.__name__))
            )
            if is_mock:
                error_msg = (
                    f"CRITICAL: Database query returned a mock object! "
                    f"This means the session is a mock. "
                    f"Check that get_basket_session() returns a real AsyncSession."
                )
                logger.error(error_msg)
                raise ValueError(error_msg)
    ...
    async def create_basket(self, basket: ShoppingCart) -> ShoppingCart:
        ...
        try:
            ...
            # Validate that we got a real ORM object, not a mock
            is_mock = (
                hasattr(basket_orm_loaded, '_mock_name') or 
                hasattr(basket_orm_loaded, '_spec_class') or
                (hasattr(basket_orm_loaded, '__class__') and 'Mock' in str(basket_orm_loaded.__class__.__name__))
            )
            if is_mock:
                error_msg = (
                    f"CRITICAL: Database query returned a mock object in create_basket! "
                    f"This means the session is a mock. "
                    f"Check that get_basket_session() returns a real AsyncSession."
                )
                logger.error(error_msg)
            ...
            for item in basket.items:
                if item.id not in existing_item_ids:
                    ...
                    item_orm = ShoppingCartItemORM(
                        ...
                        updated_at=basket.last_modified or basket.created_at or datetime.utcnow(),
                    )
    ...
    async def save_changes_async(self, user_name: str | None = None) -> int:
        ...
        try:
            ...
            change_count = len(self.session.new) + len(self.session.dirty) + len(self.session.deleted)
            ...
            return change_count if change_count > 0 else 1  # Return at least 1 to indicate success
    ...
    async def _orm_to_domain(self, basket_orm: ShoppingCartORM) -> ShoppingCart:
        ...
        if items_orm is None:
            ...
            logger.warning(f"Unexpected items type: {type(items_orm)}, converting to list")
