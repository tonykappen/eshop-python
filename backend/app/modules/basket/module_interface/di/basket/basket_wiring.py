...
def register_basket_handlers_with_mediator(mediator: Mediator) -> None:
    ...
    async def _register_handlers():
        ...
        from app.modules.basket.infrastructure.persistence.unit_of_work.sql_basket_unit_of_work import \
            SqlBasketUnitOfWork
        from app.modules.basket.module_interface.di.basket.basket_providers import (
            get_basket_cache_service, get_basket_mediator)
        ...
        async with session_maker() as temp_session:
            ...
            cache_service = get_basket_cache_service()
            repo = CachedBasketRepository(
                repository=sql_repo, cache_service=cache_service
            )
            ...
            mediator.register_handler(
                AddItemIntoBasketCommand,
                AddItemIntoBasketHandler(repository=repo, mediator=basket_mediator),
            )
            mediator.register_handler(
                CreateBasketCommand, CreateBasketHandler(repository=repo)
            )
            mediator.register_handler(
                DeleteBasketCommand, DeleteBasketHandler(repository=repo)
            )
            mediator.register_handler(
                RemoveItemFromBasketCommand,
                RemoveItemFromBasketHandler(repository=repo),
            )
            mediator.register_handler(
                UpdateItemPriceInBasketCommand,
                UpdateItemPriceInBasketHandler(repository=repo),
            )
            mediator.register_handler(
                CheckoutBasketCommand,
                CheckoutBasketHandler(repository=repo, outbox_service=outbox_service),
            )
