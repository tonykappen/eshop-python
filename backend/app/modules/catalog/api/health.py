...
from app.modules.catalog.api.deps import get_catalog_session
...
async def _check_required_tables(session: AsyncSession) -> dict[str, str]:
    ...
    for table in required_tables:
        # Validate table name - only allow alphanumeric, underscore, and dash
        ...
        try:
            # Table name is validated, so safe to use
            await session.execute(text(f'SELECT 1 FROM "{table}" LIMIT 1'))  # nosec B608
