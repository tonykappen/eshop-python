...
class AuthorizationBehavior(IPipelineBehavior[IRequest, Any]):
    ...
    async def handle(
        ...
        try:
            ...
            if hasattr(request, 'requires_authorization') and request.requires_authorization:
            ...
            logger.error(f"Unexpected error during authorization of {request_type}: {e}")
            raise

    async def _check_authorization(self, request: IRequest) -> bool:
        """
