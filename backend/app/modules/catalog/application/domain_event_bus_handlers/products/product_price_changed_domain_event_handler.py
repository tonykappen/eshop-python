...
class ProductPriceChangedDomainEventBusHandler:
    ...
    async def handle(self, domain_event: ProductPriceChangedDomainEvent) -> None:
        ...
        """
        logger.info(
            f"Converting price changed domain event to integration event for product {domain_event.product_id}"
        )
        ...
        try:
            ...
            if self.outbox_service:
                ...
                logger.info(
                    f"Written price changed integration event to outbox: {integration_event.event_id}"
                )
            elif self.event_publisher:
                # Direct publish (less reliable)
                await self.event_publisher.publish(integration_event)
                logger.info(
                    f"Published product price changed integration event {integration_event.event_id}"
                )
            else:
                logger.warning(
