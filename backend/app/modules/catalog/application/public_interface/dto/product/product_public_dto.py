...
class ProductPublicDto(BaseModel):
    ...
    price_currency: str = Field(default="USD", description="Product price currency (ISO 4217)")
    
    # Categorization
    categories: list[str] = Field(default_factory=list, description="Product categories")
    
    # Media
    image_file: Optional[str] = Field(None, description="Product image file path/URL")
    
    # Status and versioning
    is_active: bool = Field(default=True, description="Whether the product is active/available")
