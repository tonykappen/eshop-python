...
from app.modules.catalog.infrastructure.persistence.models.inventory_item_orm import (
    InventoryItemORM,
)
...
class InventoryRepositoryImpl(InventoryRepository):
    ...
    async def get_by_id(self, inventory_item_id: UUID) -> InventoryItem | None:
        ...
        try:
            stmt = select(InventoryItemORM).where(
                ...
                InventoryItemORM.is_deleted is False,
            )
    ...
    async def get_out_of_stock_items(self) -> list[InventoryItem]:
        ...
        try:
            stmt = select(InventoryItemORM).where(
                InventoryItemORM.quantity <= 0, InventoryItemORM.is_deleted is False
            )
            result = await self.session.execute(stmt)
            inventory_items_orm = result.scalars().all()
    ...
    async def get_all(self, skip: int = 0, limit: int = 100) -> list[InventoryItem]:
        ...
        try:
            stmt = (
                ...
                .where(InventoryItemORM.is_deleted is False)
                .offset(skip)
                .limit(limit)
            )
            result = await self.session.execute(stmt)
