...
class ProductRepositoryImpl(ProductRepository):
    ...
    async def get_by_category(self, category: str, page: int = 1, page_size: int = 10) -> List[Product] | tuple[list[Product], int]:
        """
        ...
        try:
            ...
            if page != 1 or page_size != 10:
                ...
                stmt = select(ProductORM).where(
                    ProductORM.categories.contains([category]),
                    ProductORM.is_deleted == False
                ).offset(offset).limit(page_size).order_by(ProductORM.created_at.desc())
    ...
    async def get_deleted_products(self, page: int = 1, page_size: int = 10) -> tuple[list[Product], int]:
        ...
        try:
            ...
            stmt = select(ProductORM).where(
                ProductORM.is_deleted == True
            ).offset(offset).limit(page_size).order_by(
