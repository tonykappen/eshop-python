...
class SqlInventoryRepository(InventoryRepository):
    ...
    async def get_low_stock_items(self) -> list[InventoryItem]:
        ...
        try:
            ...
            return [
                self._orm_to_domain(inventory_orm)
                for inventory_orm in inventory_items_orm
            ]

        except Exception as e:
            logger.log_error_with_context("Error getting low stock items", error=e)
            raise
    ...
    async def get_out_of_stock_items(self) -> list[InventoryItem]:
        ...
        try:
            stmt = select(InventoryItemORM).where(
                InventoryItemORM.quantity <= 0, InventoryItemORM.is_deleted == False
            )
            result = await self.session.execute(stmt)
            inventory_items_orm = result.scalars().all()
    ...
    async def get_all(self, skip: int = 0, limit: int = 100) -> list[InventoryItem]:
        ...
        try:
            stmt = (
                ...
                .where(InventoryItemORM.is_deleted == False)
                .offset(skip)
                .limit(limit)
            )
            result = await self.session.execute(stmt)
            ...
            logger.log_error_with_context("Error getting all inventory items", error=e)
            raise
    ...
    async def count(self) -> int:
        ...
        try:
            ...
            logger.log_error_with_context("Error counting inventory items", error=e)
            raise
    ...
    async def add(self, inventory_item: InventoryItem) -> None:
        ...
        try:
            ...
            logger.log_error_with_context("Error adding inventory item", error=e)
            raise
    ...
    async def update(self, inventory_item: InventoryItem) -> None:
        ...
        try:
            stmt = (
                update(InventoryItemORM)
                .where(InventoryItemORM.id == inventory_item.id)
                .values(
            ...
            logger.log_error_with_context("Error updating inventory item", error=e)
            raise
    ...
    async def delete(self, inventory_item_id: UUID) -> None:
        ...
        try:
            stmt = (
                update(InventoryItemORM)
                .where(InventoryItemORM.id == inventory_item_id)
