...
class SeedRunner:
    ...
    async def run_all_seeds(self) -> None:
        ...
        for seed in seeds:
            try:
                ...
                logger.log_with_context(
                    "Executing seed", context={"version": seed.version}
                )
                ...
                logger.log_with_context(
                    "Successfully executed seed", context={"version": seed.version}
                )

            except Exception as e:
                logger.log_error_with_context(
                    "Error executing seed", error=e, context={"version": seed.version}
                )
                raise

        logger.log_with_context(
            "Seed execution completed", context={"executed_count": executed_count}
        )
    ...
    async def run_specific_seed(self, version: str) -> None:
        ...
        if await self._is_seed_executed(version):
            logger.log_with_context(
                "Seed already executed, skipping", context={"version": version}
            )
            return

        logger.log_with_context("Executing seed", context={"version": version})
        await seed.execute(self.session)
        await self._record_seed_execution(version)
        logger.log_with_context(
            "Successfully executed seed", context={"version": version}
        )
    ...
    async def _record_seed_execution(self, version: str) -> None:
        ...
        logger.log_with_context("Recorded seed execution", context={"version": version})
