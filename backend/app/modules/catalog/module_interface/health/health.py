...
async def readiness_check(
    ...
    try:
        ...
        return {
            "status": "ready",
            "service": "catalog",
            "checks": {"database": "healthy", "tables": tables_check},
        }

    except Exception as e:
        logger.log_error_with_context("Readiness check failed", error=e)
        raise HTTPException(
...
async def liveness_check() -> dict[str, str]:
    ...
    """
    return {"status": "alive", "service": "catalog"}
    ...
    for table in required_tables:
        try:
            ...
            logger.log_warning_with_context(
                "Table check failed", context={"table": table, "error": str(e)}
            )
...
async def detailed_health_check(
    ...
    try:
        ...
        # Overall health
        overall_healthy = db_health["status"] == "healthy" and all(
            status == "exists" for status in table_health.values()
        )
        ...
        logger.log_error_with_context("Detailed health check failed", error=e)
        return {
            "status": "unhealthy",
            "service": "catalog",
            "error": str(e),
...
async def _check_database_health(session: AsyncSession) -> dict[str, Any]:
    ...
    try:
        ...
        logger.log_error_with_context("Database health check failed", error=e)
