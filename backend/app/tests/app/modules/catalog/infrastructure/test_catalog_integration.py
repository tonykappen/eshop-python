...
class TestCatalogIntegration:
    ...
    @pytest.mark.asyncio
    async def test_complete_crud_workflow_admin(
        self, client, sample_product_data, admin_headers
    ):
        ...
        update_response = await client.put(
            f"/api/v1/products/{product_id}", json=updated_data, headers=admin_headers
        )
        ...
        assert get_deleted_response.status_code in [
            404,
            500,
        ]  # Depending on error handling

    @pytest.mark.asyncio
    async def test_complete_crud_workflow_manager(
        self, client, sample_product_data, manager_headers
    ):
        ...
        update_response = await client.put(
            f"/api/v1/products/{product_id}", json=updated_data, headers=manager_headers
        )
        assert update_response.status_code == 200

        # 3. Delete the product
        delete_response = await client.delete(
            f"/api/v1/products/{product_id}", headers=manager_headers
        )
        assert delete_response.status_code == 200

    @pytest.mark.asyncio
    async def test_user_readonly_access(
        self, client, sample_product_data, user_headers
    ):
        ...
        get_products_response = await client.get(
            "/api/v1/products/", headers=user_headers
        )
    ...
    async def test_pagination_workflow(self, client, admin_headers):
        ...
        page1_response = await client.get(
            "/api/v1/products/?page=1&page_size=2", headers=admin_headers
        )
        ...
        page2_response = await client.get(
            "/api/v1/products/?page=2&page_size=2", headers=admin_headers
        )
        ...
        for product_id in products_created:
            await client.delete(f"/api/v1/products/{product_id}", headers=admin_headers)
    ...
    async def test_error_handling_workflow(self, client, admin_headers):
        ...
        invalid_uuid_response = await client.get(
            "/api/v1/products/invalid-uuid", headers=admin_headers
        )
        ...
        get_response = await client.get(
            f"/api/v1/products/{non_existent_id}", headers=admin_headers
        )
        ...
        create_response = await client.post(
            "/api/v1/products/", json=invalid_product_data, headers=admin_headers
        )
    ...
    async def test_data_validation_workflow(self, client, admin_headers):
        ...
        invalid_scenarios = [
            {"data": {"name": "", "price": 100}, "expected_status": 422},  # Empty name
            {
            ...
            {"data": {"price": 100}, "expected_status": 422},  # Missing name
            {
                "data": {
                    "name": "Test",
                    "price": 100,
        ...
        for scenario in invalid_scenarios:
            response = await client.post(
                "/api/v1/products/", json=scenario["data"], headers=admin_headers
            )
    ...
    async def test_large_data_handling(self, client, admin_headers):
        ...
        create_response = await client.post(
            "/api/v1/products/", json=large_product_data, headers=admin_headers
        )
