...
from app.core.logging.trace_context import get_trace_id, set_trace_context
...
async def test_trace_propagation():
    ...
    with open(log_file) as f:
        ...
        for i, line in enumerate(lines, 1):
            ...
            has_nested_context = "context" in log_entry and isinstance(
                log_entry.get("context"), dict
            )
            ...
            print(
                f"      trace_id: {'✅' if has_trace_id else '❌'} {log_entry.get('trace_id', 'MISSING')}"
            )
            print(
                f"      span_id: {'✅' if has_span_id else '❌'} {log_entry.get('span_id', 'MISSING')}"
            )
            print(
                f"      request_id: {'✅' if has_request_id else '❌'} {log_entry.get('request_id', 'MISSING')}"
            )
            print(
                f"      Nested context: {'❌' if not has_nested_context else '⚠️  FOUND'}"
            )
            print(
                f"      Flattened fields: {'✅' if (has_request_type or has_operation) else '❓'}"
            )
