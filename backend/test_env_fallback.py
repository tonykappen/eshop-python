...
import os
from app.config.env import get_env, get_env_bool, get_env_int, get_all_env, has_env
...
def test_env_fallback():
    ...
    for key, value in sorted(all_env.items()):
        if any(service in key.lower() for service in ["database", "redis", "rabbitmq", "keycloak", "seq", "log"]):
            print(f"  {key}: {value}")
