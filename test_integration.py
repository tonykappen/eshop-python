...
class EShopAPITester:
    ...
    def __init__(self, base_url: str = "http://localhost:8000"):
        ...
        self.test_product_id: str | None = None
    ...
    def test_health_endpoints(self) -> bool:
        ...
        try:
            ...
            if response.status_code != 200:
                self.log(
                    f"Detailed health check failed: {response.status_code}", "ERROR"
                )
                return False
    ...
    def test_unauthorized_access(self) -> bool:
        ...
        try:
            ...
            if response.status_code != 401:
                self.log(
                    f"Products endpoint should require auth: {response.status_code}",
                    "ERROR",
                )
                return False
            ...
            if response.status_code != 401:
                self.log(
                    f"Create product should require auth: {response.status_code}",
                    "ERROR",
                )
                return False
...
def main():
    ...
    if success:
        ...
        print("   • Postman Collection: EShop_Postman_Collection_Updated.json")
        sys.exit(0)
    else:
