...
def test_no_traceparent_header():
    ...
    print(f"  parent_span_id: {trace_ctx.parent_span_id} ← None (correct for root span)")
    print(f"  trace_flags: {trace_ctx.trace_flags}")
...
def test_trace_propagation():
    ...
    print(f"  parent_span_id: {trace_ctx_2.parent_span_id} ← POPULATED with Request 1's span_id!")
    
    # Verify parent-child relationship
    assert trace_ctx_2.trace_id == trace_ctx_1.trace_id, "Trace ID should be the same"
    assert trace_ctx_2.parent_span_id == trace_ctx_1.span_id, "Parent span ID should match Request 1's span ID"
    print("\n✅ Parent-child relationship verified!")
    print(f"   Request 1 span_id ({trace_ctx_1.span_id}) = Request 2 parent_span_id ({trace_ctx_2.parent_span_id})")
