...
class TestIntegrationEvent:
    ...
    def test_integration_event_default_values(self) -> None:
        ...
        assert event.topic == "eshop.messaging.integration"
        assert event.source_module == "messaging"
        assert event.routing_key == "eshop.messaging.integration"
        assert event.data == {}
    ...
    def test_extract_module_name_from_path(self) -> None:
        ...
        with patch.object(
            TestEvent, "__module__", "eshop.modules.catalog.domain.events"
        ):
            event = TestEvent()
    ...
    def test_extract_module_name_fallback(self) -> None:
        ...
        with patch.object(TestEvent, "__module__", "eshop.core.messaging"):
            event = TestEvent()
    ...
    def test_generate_topic(self) -> None:
        ...
        with patch.object(
            ...
            event = TestEvent()
            assert event.topic == "eshop.catalog.test"
    ...
    def test_generate_routing_key(self) -> None:
        ...
        with patch.object(
            ...
            event = TestEvent()
            assert event.routing_key == "eshop.catalog.test"
    ...
    def test_extract_module_from_path(self) -> None:
        ...
        assert (
            IntegrationEvent._extract_module_from_path(
                "eshop.modules.catalog.domain.events"
            )
            == "catalog"
        )
        assert (
            IntegrationEvent._extract_module_from_path(
                "eshop.modules.basket.domain.events"
            )
            == "basket"
        )
        assert (
            IntegrationEvent._extract_module_from_path("eshop.core.messaging") == "core"
        )
...
class TestModuleIntegrationEvent:
    ...
    def test_module_integration_event(self) -> None:
        ...
        assert event.topic == "eshop.messaging.module_integration"
        assert event.routing_key == "eshop.test_module.module_integration"
    ...
    def test_catalog_integration_event(self) -> None:
        ...
        assert event.source_module == "catalog"
        assert event.topic == "eshop.messaging.catalog_integration"
        assert event.routing_key == "eshop.catalog.catalog_integration"
    ...
    def test_basket_integration_event(self) -> None:
        ...
        assert event.topic == "eshop.messaging.basket_integration"
        assert event.routing_key == "eshop.basket.basket_integration"
    ...
    def test_ordering_integration_event(self) -> None:
        ...
        assert event.topic == "eshop.messaging.ordering_integration"
        assert event.routing_key == "eshop.ordering.ordering_integration"
...
class TestEventBus:
    ...
    def test_register_module_handlers(self) -> None:
        ...
        assert event_bus.routing_patterns["event1"] == "eshop.test_module.event1"
        assert event_bus.routing_patterns["event2"] == "eshop.test_module.event2"
    ...
    async def test_publish_event(self) -> None:
        ...
        with patch(
            "eshop.core.messaging.integration_event.FastStreamEventPublisher"
